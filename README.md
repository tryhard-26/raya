...
Raya is a fast, memory-safe static detection and binary pattern-matching engine engineered in Rust. Inspired by YARA, Raya combines multi-pattern byte matching, regular expressions, and deep executable format inspection (PE32/PE32+ and ELF32/ELF64) with section-level Shannon entropy calculations.

Unlike conventional signature engines that yield opaque binary verdicts, Raya enforces **explainability**: every match produces an evidence trace detailing the exact string offsets, imported APIs, section characteristics, or entropy metrics that satisfied the rule conditions.
...
* Rust 1.75+ and Cargo (`curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh`)

### Building from Source

```bash
git clone https://github.com/raya-security/raya.git
cd raya
cargo build --release
```

The optimized binary is generated at `target/release/raya`.
...
Stores analytical attributes attached to the detection:
