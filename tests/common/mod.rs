...
pub fn build_mock_pe32(
    sections: &[(&str, u32, &[u8])],
    imported_dlls: &[(&str, &[&str])],
) -> Vec<u8> {
    let mut pe = vec![0u8; 1024];
    ...
    // COFF File Header at 0x84
    let coff_offset = nt_offset + 4;
    pe[coff_offset..coff_offset + 2].copy_from_slice(&0x014Cu16.to_le_bytes()); // i386
    let num_sections = sections.len() as u16;
    pe[coff_offset + 2..coff_offset + 4].copy_from_slice(&num_sections.to_le_bytes());
    pe[coff_offset + 16..coff_offset + 18].copy_from_slice(&224u16.to_le_bytes()); // size of optional header (224 for PE32)
    pe[coff_offset + 18..coff_offset + 20].copy_from_slice(&0x0002u16.to_le_bytes()); // executable

    // Optional Header (PE32) at 0x98
    let opt_offset = coff_offset + 20;
    pe[opt_offset..opt_offset + 2].copy_from_slice(&0x010Bu16.to_le_bytes()); // PE32
    pe[opt_offset + 16..opt_offset + 20].copy_from_slice(&0x1000u32.to_le_bytes()); // AddressOfEntryPoint
    pe[opt_offset + 28..opt_offset + 32].copy_from_slice(&0x00400000u32.to_le_bytes()); // ImageBase

    // Data Directory for Import Table at opt_offset + 96 + 8 (index 1)
    let import_dir_offset = opt_offset + 96 + 8;
    if !imported_dlls.is_empty() {
        pe[import_dir_offset..import_dir_offset + 4].copy_from_slice(&0x2000u32.to_le_bytes());
        pe[import_dir_offset + 4..import_dir_offset + 8].copy_from_slice(&100u32.to_le_bytes());
    }

    // Section Headers start at opt_offset + 224
    let sec_table_offset = opt_offset + 224;
    let mut current_rva = 0x1000u32;
    let mut current_file_offset = 0x400u32;
...
pub fn build_mock_elf(payload: &[u8]) -> Vec<u8> {
    let mut elf = vec![0u8; 64 + payload.len()];
